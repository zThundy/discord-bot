...
export function GetCommand(command) {
    return paths.filter(path => path.module == command)
    // if (paths.find(command)) {
    //     return paths.find(command).path
    // }
    // return null
}
    ...
    client.on("messageReactionAdd", async (reactionMessage, user) => {
        try {
            ...
            paths.forEach(async file => {
                // da cambiare assolutamente e da rendere modulare
                // let command = file.path.match(/\/(.*?).js/)[1]
